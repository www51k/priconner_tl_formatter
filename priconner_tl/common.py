...
def _load_aliases() -> tuple[dict[str, str], dict[str, str]]:
    for path in (Path("/home/pyodide/character_aliases.json"), Path(__file__).resolve().parent / "data" / "character_aliases.json", Path(__file__).resolve().parent.parent / "data" / "character_aliases.json"):
        try:
...
def _load_boss_names() -> set[str]:
    for path in (Path("/home/pyodide/boss_names.json"), Path(__file__).resolve().parent / "data" / "boss_names.json", Path(__file__).resolve().parent.parent / "data" / "boss_names.json"):
        try:
            payload = json.loads(path.read_text(encoding="utf-8"))
